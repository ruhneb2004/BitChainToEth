...
const SwapComponentBTW: React.FC = () => {
  ...
  const handleWBTCChange = (value: string) => {
    ...
    if (Number(value) > 0) {
      const btcAmount = Number(value) / (1 - 0.3 / 100);
...
const SwapAmount: React.FC<TransactionAmountComponentProps> = ({
  ...
  return (
    <div className="swap-component-middle-section">
      ...
      />
      <InputField id="wbtc" label="Receive WBTC" value={btcAmount} readOnly />
    </div>
  );
};
...
export default SwapComponentBTW;
